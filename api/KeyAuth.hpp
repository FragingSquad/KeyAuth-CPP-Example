...
namespace KeyAuth {
	...
	public:
		...
		std::string name, ownerid, secret, version, seller_secret;

		api(std::string name, std::string ownerid, std::string secret, std::string version, std::string seller_secret = "")
			: name(name), ownerid(ownerid), secret(secret), version(version), seller_secret(seller_secret) {}
		 ...
		 * deluser
		 * 
		 * Delete user : 'username' using seller api
		 * 
		 * \param username
		 */
		void deluser(std::string username) {

			std::string url = XorStr("https://keyauth.com/api/seller/?sellerkey=").c_str();
			url.append(seller_secret);
			url.append(XorStr("&type=deluser&user=").c_str());
			url.append(username);
		...
		/**
		 * Add user : 'username' with subscription 'subscription' having 'expiry' as an expiration in day from now with optional password 'password' using seller api
		 * 
		 * \param username
		 * \param subscription
		 * \param expiry
		 * \param password
		 */
		void adduser(std::string username, std::string subscription, int expiry, std::string password = "") {

			std::string url = XorStr("https://keyauth.com/api/seller/?sellerkey=").c_str();
			url.append(seller_secret);
			url.append(XorStr("&type=adduserr&user=").c_str());
			url.append(username);
			url.append(XorStr("&sub=").c_str());
			url.append(subscription);
			url.append(XorStr("&expiry=").c_str());
			url.append(std::to_string(expiry));
			if (password != "") {
				url.append(XorStr("&pass=").c_str());
				url.append(password);
			}
			auto response = get(url);
			auto json = response_decoder.parse(response);
		...
		/**
		 * Reset the user : 'username' using seller api
		 * 
		 * \param username
		 */
		void resetuser(std::string username) {

			std::string url = XorStr("https://keyauth.com/api/seller/?sellerkey=").c_str();
			url.append(seller_secret);
			url.append(XorStr("&type=resetuser&user=").c_str());
			url.append(username);
			auto response = get(url);
			auto json = response_decoder.parse(response);
			if (json[("success")])
		...
		/**
		 * Edit user 'username' variable : 'var' using seller api
		 * 
		 * \param username
		 * \param varname
		 * \param vardata
		 */
		void setvarSeller(std::string username, std::string varname, std::string vardata) {

			std::string url = XorStr("https://keyauth.com/api/seller/?sellerkey=").c_str();
			url.append(seller_secret);
			url.append(XorStr("&type=setvar&user=").c_str());
			url.append(username);
			url.append(XorStr("&var=").c_str());
			url.append(varname);
			url.append(XorStr("&data=").c_str());
			url.append(vardata);
			auto response = get(url);
			auto json = response_decoder.parse(response);
			if (json[("success")])
		...
		/**
		 * Chatsend
		 * Envoie un message 'msg' dans le channel 'channelname'
	...
	private:
		...
		static std::string post(std::string data) {
